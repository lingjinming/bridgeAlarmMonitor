...
<script>
  ...
  export default {
    ...
    data(){
      return{
        parentTitle:'报警反馈',
        parentResultTit:'反馈结果:',
        parentOpinionTitle:'反馈意见:',
        parentTypeOptions: null,
        parentDefaultOption:'确认报警',
        parentStartDateTit:'反馈时间:',
        showOpinion:false,
        showUpload:false,
        showEndDate:false,
    ...
    methods:{
      ...
      comfirmSubmit(){
        ...
        formData.append('backstate',Storage.get('tempParentDefaultOption'))
        formData.append('backtime',_this.$refs.Form.tempStartDate)
        formData.append('backperson',encodeURI(encodeURI(_this.$store.getters.getUserName)))
        var filrarr = document.getElementById("fileUpload").files;
        for(var i = 0; i < filrarr.length; i++){
          formData.append("files", filrarr[i]);
        }
        $.ajax({
          url: _this.$store.getters.getAppTempUrlForJq + 'setStateFeedback.mvc',
          type: 'POST',
          cache: false,
          data: formData,
          processData: false,
        ...
        //
        // _this.$axios({
        //   url: 'setStateFeedback.mvc',  //请求路径（接口）
        //   method: 'POST', //请求方式
